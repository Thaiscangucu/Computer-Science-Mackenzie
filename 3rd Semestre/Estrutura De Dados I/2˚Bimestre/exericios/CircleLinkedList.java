...
public class CircleLinkedList<T> {
	...
	public void concatena(CircleLinkedList lista){
	...
	public boolean remove(T data){
	    ...
	    if (isEmpty()) return false; // Se lista vazia, não é possível remover
	      ...
	      // procura a posição do elemento na lista
	      int contador = 1;
	      while ((contador != size) && (!pAnda.getDado().equals(data))){
	        pAnt = pAnda;
	        pAnda = pAnda.getProx();
	        contador++;
	      }
	      if ((contador == size) && (!pAnda.getDado().equals(data)))
	...
	public String toString() {
	    ...
	    if (size != 0) {
	    	sb.append("\nPrimeiro: ").append(head.getDado()).append(", Ultimo: ").append(tail.getDado());
			System.out.println("\n");
	    }
